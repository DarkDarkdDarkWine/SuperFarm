...
奶牛 (Cow):    1面  (8.3%)
狐狸 (Fox):    1面  (8.3%)
狼 (Wolf):     0面  (0% - 仅在困难模式启用)
```

**重要说明**：
- **马不在骰子上**，只能通过交换动物获得
- **狼**在经典模式中默认不启用，可在困难模式中开启
...
**繁殖公式**：
```
新动物总数 = floor((现有数量 + 骰子数量) / 2)
```

**重要说明**：
- **所有**动物（包括现有的和骰子显示的）都参与繁殖计算
- 繁殖后的数量是**新的总数**，不是增加的数量
- 即使骰子没掷出某种动物，只要你持有成对数量，依然会繁殖

**繁殖示例**：

| 现有数量 | 骰子掷出 | 合计 | 繁殖计算 | 最终数量 |
|---------|---------|------|---------|---------|
| 1只兔子 | 0只兔子 | 1 | floor(1/2) = 0 | **0只** ❌ |
| 1只兔子 | 1只兔子 | 2 | floor(2/2) = 1 | **1只** |
| 3只兔子 | 0只兔子 | 3 | floor(3/2) = 1 | **1只** ⬇️ |
| 3只兔子 | 1只兔子 | 4 | floor(4/2) = 2 | **2只** ⬇️ |
| 5只兔子 | 1只兔子 | 6 | floor(6/2) = 3 | **3只** ⬇️ |
| 2只羊 | 2只羊 | 4 | floor(4/2) = 2 | **2只** |
| 4只猪 | 0只猪 | 4 | floor(4/2) = 2 | **2只** ⬇️ |

**关键理解**：
```
❗ 繁殖不是"增加"动物，而是"重新计算"总数
❗ 奇数动物在繁殖时会"损失"1只（因为向下取整）
❗ 这就是为什么要先交换再掷骰子——避免奇数动物的损失
```
...
决策A（不交换）：
→ 掷骰子得1只兔子
→ 繁殖计算：floor((7+1)/2) = floor(8/2) = 4只
→ 结果：7只变成4只 ❌ 亏损！

决策B（先交换）：
→ 交换：6只兔子换1只羊 → 剩1只兔子、1只羊
→ 掷骰子得1只兔子
→ 兔子繁殖：floor((1+1)/2) = 1只
→ 羊繁殖：floor((1+0)/2) = 0只 → 羊消失！❌

决策C（最优策略）：
→ 交换：6只兔子换1只羊 → 剩1只兔子、1只羊
→ 购买1只小狗（用1只兔子）→ 剩0只兔子、1只羊、1只小狗
→ 掷骰子得1只兔子
→ 兔子繁殖：floor((0+1)/2) = 0只
→ 羊繁殖：floor((1+0)/2) = 0只
→ 全灭！❌❌❌ 这就是经典版的硬核之处！
...
| 当前数量 | 最佳行动 | 原因 |
|---------|---------|------|
| 0只 | 寄希望骰子掷出≥2只 | 0只无法繁殖 |
| 1只 | 希望骰子掷出≥1只 | 变成偶数 |
| 2只 | 保持，等待骰子 | 稳定繁殖 |
| 3只 | **警告**：尽快交换或等骰子 | 奇数会损失 |
| 4只 | 理想状态 | 稳定繁殖 |
| 5只 | 尽快交换掉1只 | 奇数危险 |
| 6只 | 可以考虑交换升级 | 升级良机 |
| 7只+ | 奇数交换，偶数保持 | 管理数量 |
...
**效果**：
- **目标**：对手玩家
- **惩罚**：对手失去**所有羊、猪、牛**（马和兔子保留）

**防御**：
- 如果对手拥有**大狗**，消耗1只大狗完全抵消攻击
- 没有大狗时，羊、猪、牛全部归还银行

**示例**：
```
对手状态：10兔、5羊、3猪、1牛、1马、0只大狗
你掷出：1只狼
结果：对手变成 → 10兔、0羊、0猪、0牛、1马
（羊、猪、牛全部失去）

---

对手状态：10兔、5羊、3猪、1牛、1马、1只大狗
你掷出：1只狼
结果：对手消耗1只大狗 → 10兔、5羊、3猪、1牛、1马、0只大狗
（其他动物不受影响）
```
...
const newRabbits = calculateBreeding(currentRabbits, diceRabbits);
// 计算：floor((5+1)/2) = floor(6/2) = 3
// 结果：玩家的兔子数量变为3只

console.log(`繁殖前：${currentRabbits}只 + 骰子${diceRabbits}只 = ${currentRabbits + diceRabbits}只`);
console.log(`繁殖后：${newRabbits}只`);
console.log(`变化：${newRabbits - currentRabbits}只（${newRabbits > currentRabbits ? '增加' : '减少'}）`);
```
...
| 结果 | 面数 | 单骰概率 | 双骰至少1次 | 双骰出现2次 |
|-----|------|---------|-----------|-----------|
| 兔子 | 6/12 | 50.0% | 75.0% | 25.0% |
| 羊 | 2/12 | 16.7% | 30.6% | 2.8% |
| 猪 | 2/12 | 16.7% | 30.6% | 2.8% |
| 牛 | 1/12 | 8.3% | 16.0% | 0.7% |
| 狐狸 | 1/12 | 8.3% | 16.0% | 0.7% |
