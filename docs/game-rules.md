...
**繁殖前提条件**：
- 条件A：骰子掷出该动物 **且** 玩家已持有该动物至少1只 → 正常繁殖
- 条件B（双同例外）：两颗骰子**同时**掷出同一种动物，即使玩家持有0只 → 获得1只
- 两个条件都不满足则跳过
...
- 骰子没掷出的动物不会繁殖（即使你有很多）
- 获得数量受银行库存限制
- 牛只出现在骰子A，马只出现在骰子B，因此不可能掷出双牛或双马，**第一只牛/马必须通过交换获得**
...
**繁殖前提条件**：
- 条件A：骰子掷出该动物 **且** 已持有该动物至少1只 → 正常繁殖
- 条件B（双同例外）：两颗骰子同时掷出同种动物，即使持有0只 → 获得1只
- 牛/马无法触发双同例外（分别只出现在一颗骰子上）
...
**效果**：
- **目标**：掷出狐狸的玩家自己（攻击自己）
- **惩罚**：兔子减少到只剩**1只**（被吃掉的兔子归还银行）

**防御**：
- 使用**小狗**：消耗1只小狗完全抵消攻击，兔子不受影响
- **大狗无法防御狐狸**
- 没有小狗时，兔子必然受到惩罚
...
**效果**：
- **目标**：掷出狼的玩家自己（攻击自己）
- **惩罚**：失去**所有羊、猪、牛**（兔子和马保留）

**防御**：
- 如果拥有**大狗**，消耗1只大狗完全抵消攻击
- 没有大狗时，羊、猪、牛全部归还银行（兔子和马不受影响）

**示例**：
```
你的状态：10兔、5羊、3猪、1牛、1马、0只大狗
你掷出：1只狼
结果：你变成 → 10兔、0羊、0猪、0牛、1马
（羊、猪、牛全部失去；兔子和马保留）
...
```
示例：你同时掷出狐狸+狼
你的状态：15只兔子、5只羊、2只猪、1只牛、1只马、0只小狗、0只大狗

结算过程：
1. 狐狸 → 无小狗防御，兔子减到1只
2. 狼 → 无大狗防御，羊/猪/牛全部归还银行（兔子和马不受狼影响）
最终：1只兔子、0只羊、0只猪、0只牛、1只马

---

你的状态：15只兔子、5只羊、1只小狗、0只大狗
掷出狐狸+狼：
1. 狐狸 → 消耗小狗，兔子保留15只
2. 狼 → 无大狗防御，羊全部归还银行（兔子不受狼影响）
最终：15只兔子、0只羊、0只小狗
```
...
骰子A：兔6、羊3、猪1、牛1、狐1
骰子B：兔6、羊3、猪1、马1、狼1
攻击：狐狸减到只剩1只兔子（小狗防御）；狼清空羊、猪、牛（兔子和马保留，大狗防御）
防御：小狗只防狐狸，大狗只防狼
初始：每人1只兔子
胜利：集齐5种动物
回合上限：50回合
...
骰子A：兔6、羊3、猪1、牛1、狐1
骰子B：兔6、羊3、猪1、马1、狼1
攻击：同经典模式（狐狸剩1只兔子；狼清空羊/猪/牛，兔子和马保留）
初始：每人2只兔子
胜利：集齐5种动物
回合上限：30回合
```
